...
class GridStack {
    ...
    _updateStyles(forceUpdate = false, maxH) {
        ...
        this.getGridItems().forEach((w) => {
            if (w.gridstackNode._dirty) {
                utils_1.Utils.updatePositionStyleOnWidget(w, this.opts.cellHeight, this.opts.cellHeightUnit);
            }
        });
    ...
    _updateElementChildrenStyling(el, prefix) {
        ...
        if (el !== undefined) {
            ...
            // content margins
            utils_1.Utils.updateStyleOnElements([el.querySelector(content)], { top, right, bottom, left });
            utils_1.Utils.updateStyleOnElements(placeholder, { top, right, bottom, left });
            // resize handles offset (to match margin)
            utils_1.Utils.updateStyleOnElements([el.querySelector(`.ui-resizable-ne`)], { right });
            utils_1.Utils.updateStyleOnElements([el.querySelector(`.ui-resizable-e`)], { right });
            utils_1.Utils.updateStyleOnElements([el.querySelector(`.ui-resizable-se`)], { right, bottom });
            utils_1.Utils.updateStyleOnElements([el.querySelector(`.ui-resizable-nw`)], { left });
            utils_1.Utils.updateStyleOnElements([el.querySelector(`.ui-resizable-w`)], { left });
            utils_1.Utils.updateStyleOnElements([el.querySelector(`.ui-resizable-sw`)], { left, bottom });
        }
        if (prefix != undefined) {
            content = `${prefix} > .grid-stack-item-content`;
            // content margins
            utils_1.Utils.updateStyleOnElements(content, { top, right, bottom, left });
            utils_1.Utils.updateStyleOnElements(placeholder, { top, right, bottom, left });
            // resize handles offset (to match margin)
            utils_1.Utils.updateStyleOnElements(`${prefix} > .ui-resizable-ne`, { right });
            utils_1.Utils.updateStyleOnElements(`${prefix} > .ui-resizable-e`, { right });
            utils_1.Utils.updateStyleOnElements(`${prefix} > .ui-resizable-se`, { right, bottom });
            utils_1.Utils.updateStyleOnElements(`${prefix} > .ui-resizable-nw`, { left });
            utils_1.Utils.updateStyleOnElements(`${prefix} > .ui-resizable-w`, { left });
            utils_1.Utils.updateStyleOnElements(`${prefix} > .ui-resizable-sw`, { left, bottom });
        }
    }
    /** @internal */
